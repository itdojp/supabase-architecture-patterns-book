...
---
# 第5-2章：マルチテナンシーと複雑ビジネスロジック

---
- **目次に戻る**: [はじめに]({{ '/introduction/' | relative_url }})
- **前の章**: [第5-1章：パターン3 - 独立 API サーバー]({{ '/chapters/chapter05-1/' | relative_url }})
- **次の章**: [第5-3章：拡張性設計とパフォーマンス最適化]({{ '/chapters/chapter05-3/' | relative_url }})
- **アーキテクチャ**: 独立 API サーバー（FastAPI + マルチテナント）
- **学習レベル**:  基礎 |  応用 |  発展
- **推定学習時間**: 4〜6時間
...
## この章で扱う構成
- 構成: 独立 API サーバー + マルチテナント
- 推奨用途: SaaS/組織管理/複数テナント運用
- 非推奨用途: 単一テナントで十分なケース

## この章で学ぶこと（初心者向け）

この章では、第5-1章で構築したSaaSプラットフォームに、**「マンション管理」**のような高度な機能を追加します。
...
flowchart TD
    ...
    B --> E[ テナント分離層<br/>Tenant Middleware]
...
class TenantMiddleware(BaseHTTPMiddleware):
    ...
    def _extract_organization_id(self, request: Request):
        ...
        # パターン2: URL パスから（例: /api/v1/orgs/123/projects）
...
| `X-Organization-ID` | HTTP ヘッダーで企業IDを指定 | 「私はA社の田中です」という名札 |
| `request.state.organization` | 後続処理で使える企業情報を保存 | 「この人はA社の人」というメモを貼り付け |
| `path.split("/")`| URL から企業IDを抜き出し | 住所「東京都-A区-123番地」から123を抜き出し |
...
class ProjectService:
    ...
    def _generate_recommendations(self, completion_rate: float, on_time_rate: float, tasks_per_member: float) -> List[str]:
        ...
        if completion_rate < 50:
            recommendations.append("[WARN] タスク完了率が低下しています。チームの負荷状況を確認してください")
        ...
        if not recommendations:
            recommendations.append("[OK] プロジェクトは健全に進行しています")
    ...
    async def get_project_timeline_analysis(self, project: Project) -> Dict[str, Any]:
        ...
        milestone_tasks = self.db.query(Task).filter(
            and_(
                Task.project_id == project.id,
                Task.tags.contains(['milestone'])  # JSON フィールドの検索
            )
        ).all()
...
def generate_report_email_template(organization: Organization, report_data: Dict[str, Any]) -> str:
    ...
    if summary['at_risk_projects'] > 0:
        html += f"<p>[WARN] {summary['at_risk_projects']}件のプロジェクトが要注意状態です。詳細を確認してください。</p>"
    else:
        html += "<p>[OK] すべてのプロジェクトが順調に進行しています。</p>"
...
第5-2章では、マルチテナント機能と複雑なビジネスロジックの実装を行いました。
...
### [OK] **習得できたスキル**
- [OK] エンタープライズ級マルチテナント・アーキテクチャ設計
- [OK] 複雑ビジネスロジックのサービス層による整理・実装
- [OK] バックグラウンド処理・非同期タスクキューシステム
- [OK] 高度な権限制御・データ分離・セキュリティ実装

### **マルチテナント実装の進歩**
| 機能要素 | 第5-1章 (基礎) | 第5-2章 (発展) | 適用効果 |
|:---------|:------------------|:------------------|:---------|
| **データ分離**|  基本 RLS |  完全テナント分離 | 企業間データ漏洩防止 |
| **権限管理**|  シンプルRBAC |  階層組織・複雑権限 | 大企業組織構造対応 |
| **処理性能**|  同期処理 |  非同期・バックグラウンド | 大量データ処理対応 |
| **分析機能**| [NG] なし |  自動レポート・健全性分析 | 経営判断支援 |

### **次の学習ステップ**
第5-3章で学ぶ拡張性設計とパフォーマンス最適化の前提知識：
- [OK] マルチテナント・アーキテクチャの理解（スケーリング戦略への応用）
- [OK] 非同期処理・タスクキューの実装経験（パフォーマンス最適化への拡張）
- [OK] 複雑ビジネスロジックの設計経験（運用監視への応用）
- [OK] Redisキャッシュ活用経験（高性能システムへの発展）

---

## 次章予告：拡張性設計とパフォーマンス最適化

**第5-3章**では、「**国際空港の運用管理システム**」レベルの本番対応機能を実装します：
...
- **前の章**: [第5-1章：パターン3 - 独立 API サーバー]({{ '/chapters/chapter05-1/' | relative_url }})
- **次の章**: [第5-3章：拡張性設計とパフォーマンス最適化]({{ '/chapters/chapter05-3/' | relative_url }})
- **関連章**: [第6章：パフォーマンス最適化]({{ '/chapters/chapter06/' | relative_url }}) | [第8章：運用監視と自動化]({{ '/chapters/chapter08/' | relative_url }})
